...
When applying a changeset, (assessing the changeset, or creating the original comparison filesystem) symlinks that are either:
...
As an example, umoci provides a flag `--keep-dirlinks`. 
...
Link replacement in a later layer SHOULD be replaced, following the same convention as a changeset over existing files.
